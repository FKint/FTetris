...
float* print(const font_data &ft_font, float x, float y, const char *fmt, ...)  {
	...
	GLboolean *lighting, *texture2d, *depthtest, *blend;
	lighting = new GLboolean;
	texture2d = new GLboolean;
	depthtest = new GLboolean;
	blend = new GLboolean;
	...
	toreturn[0] = ((float)maxlen)*1.0;//width
	toreturn[1] = ((float)lines.size())*1.0;//linesamount
	toreturn[2] = ((float)h)*1.0;//height per line
	return toreturn;
}
